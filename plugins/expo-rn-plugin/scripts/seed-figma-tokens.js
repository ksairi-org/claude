 ...
 *
 * Requires a blank Figma file. Get the file key from the URL:
...
if (!FILE_ID || !TOKEN) {
  console.error("Error: FIGMA_FILE_ID and FIGMA_API_KEY must be set.");
  process.exit(1);
}
...
function buildPayload() {
  ...
  variableModes.push(
    { action: "CREATE", id: "mode_light", name: "Light", variableCollectionId: "col_colors" },
    { action: "CREATE", id: "mode_dark",  name: "Dark",  variableCollectionId: "col_colors" }
  );

  for (const name of Object.keys(lightColors)) {
    const varId = `var_color_${name.replace(/[^a-zA-Z0-9]/g, "_")}`;
    variables.push({
      action: "CREATE",
      id: varId,
      name,
      variableCollectionId: "col_colors",
      resolvedType: "COLOR",
    });
    variableModeValues.push(
      { variableId: varId, modeId: "mode_light", value: parseColor(lightColors[name]) },
      { variableId: varId, modeId: "mode_dark",  value: parseColor(darkColors[name] ?? lightColors[name]) }
    );
  }
  ...
  variableCollections.push({
    ...
    id: "col_sizes",
    name: "Sizes & Spaces",
    initialModeId: "mode_sizes",
  });
  variableModes.push({
    action: "CREATE",
    id: "mode_sizes",
    name: "Default",
    variableCollectionId: "col_sizes",
  });

  for (const [name, value] of Object.entries(sizes)) {
    const varId = `var_size_${name.replace(/[^a-zA-Z0-9]/g, "_")}`;
    variables.push({
      action: "CREATE",
      id: varId,
      name,
    ...
    variableModeValues.push({ variableId: varId, modeId: "mode_sizes", value });
  }
  ...
  variableCollections.push({
    ...
    id: "col_radius",
    name: "Radius",
    initialModeId: "mode_radius",
  });
  variableModes.push({
    action: "CREATE",
    id: "mode_radius",
    name: "Default",
    variableCollectionId: "col_radius",
  });

  for (const [name, value] of Object.entries(radius)) {
    const varId = `var_radius_${name.replace(/[^a-zA-Z0-9]/g, "_")}`;
    variables.push({
      action: "CREATE",
      id: varId,
      name,
    ...
    variableModeValues.push({ variableId: varId, modeId: "mode_radius", value });
  }
  ...
  variableCollections.push({
    ...
    id: "col_font_sizes",
    name: "Font Sizes",
    initialModeId: "mode_font_sizes",
  });
  variableModes.push({
    action: "CREATE",
    id: "mode_font_sizes",
    name: "Default",
    variableCollectionId: "col_font_sizes",
  });

  for (const [name, value] of Object.entries(fontSizes)) {
    const varId = `var_fontsize_${name}`;
    variables.push({
      action: "CREATE",
      id: varId,
      name: `size-${name}`,
    ...
    variableModeValues.push({ variableId: varId, modeId: "mode_font_sizes", value });
  }
  ...
  variableCollections.push({
    ...
    id: "col_line_heights",
    name: "Line Heights",
    initialModeId: "mode_line_heights",
  });
  variableModes.push({
    action: "CREATE",
    id: "mode_line_heights",
    name: "Default",
    variableCollectionId: "col_line_heights",
  });

  for (const [name, value] of Object.entries(lineHeights)) {
    const varId = `var_lh_${name}`;
    variables.push({
      action: "CREATE",
      id: varId,
    ...
    variableModeValues.push({ variableId: varId, modeId: "mode_line_heights", value });
  }
...
async function main() {
  console.log(`Seeding Figma file: ${FILE_ID}`);

  const payload = buildPayload();
  console.log(
    `Creating: ${payload.variableCollections.length} collections, ` +
    `${payload.variables.length} variables`
  );

  const res = await fetch(
    `https://api.figma.com/v1/files/${FILE_ID}/variables`,
    {
      method: "POST",
      headers: {
        "X-Figma-Token": TOKEN,
  ...
  console.log(
    `Open: https://www.figma.com/file/${FILE_ID} and check the Variables panel.`
  );
}
