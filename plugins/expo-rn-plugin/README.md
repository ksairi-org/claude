...
```bash
claude --plugin-dir /path/to/expo-rn-plugin
```

Or add to a project permanently:

```bash
claude plugin install expo-rn-plugin@your-marketplace --scope project
```

On first load the plugin builds the MCP servers into a persistent data directory automatically (requires Node.js and npm).
...
| `PreToolUse` (Bash) | `rtk-rewrite.sh` | RTK token optimizer — 60–90% token savings on shell commands |
...
These replace the legacy `.mcp-project` / `.mcp-env` files. Both file-based values are still read as a fallback for projects bootstrapped before the plugin.

## Project CLAUDE.md

When using this plugin, keep your project's `CLAUDE.md` lean (under 50 lines). Move detailed standards to the on-demand skill:
...
Projects previously using `scripts/bootstrap.sh` continue to work — `bin/mcp-run.sh` reads `.mcp-project` / `.mcp-env` as a fallback. To migrate fully:

1. Install the plugin: `claude plugin install expo-rn-plugin@marketplace --scope project`
2. Configure it: `/plugin configure expo-rn-plugin` (enter your Doppler project/config)
3. Remove the old `.mcp.json` from your project — the plugin provides it
4. Trim your `CLAUDE.md` to ≤50 lines and add the one-liner pointing to `coding-standards`

## Adding a new MCP server

1. Create a directory at the plugin root (e.g. `my-mcp-server/`)
2. Follow the structure of `expo-mcp-server/` (`src/index.ts`, `src/tools/`, `package.json`, `tsconfig.json`)
